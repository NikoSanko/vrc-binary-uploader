    ...
    B::Error: Into<Box<dyn Error + Send + Sync>>,
{
        ...
        context: &C) -> Result<PingResponse, ApiError>
    {
        ...
        match response.status().as_u16() {
            200 => {
                ...
                Ok(PingResponse::SuccessfulOperation
                    (body)
                )
            }
            500 => {
                ...
                Ok(PingResponse::InternalServerError
                    (body)
                )
            }
            code => {
        ...
        context: &C) -> Result<UpdateMergedImageResponse, ApiError>
    {
        ...
        let (body_string, multipart_header) = {
            ...
            let signed_url_str = match serde_json::to_string(&param_signed_url) {
                Ok(str) => str,
                Err(e) => return Err(ApiError(format!("Unable to serialize signed_url to string: {e}"))),
            };

            let signed_url_vec = signed_url_str.as_bytes().to_vec();
            let signed_url_mime = mime::Mime::from_str("application/json").expect("impossible to fail to parse");
            let signed_url_cursor = Cursor::new(signed_url_vec);

            multipart.add_stream("signed_url",  signed_url_cursor,  None as Option<&str>, Some(signed_url_mime));


            let index_str = match serde_json::to_string(&param_index) {
                Ok(str) => str,
                Err(e) => return Err(ApiError(format!("Unable to serialize index to string: {e}"))),
            };

            let index_vec = index_str.as_bytes().to_vec();
            let index_mime = mime::Mime::from_str("application/json").expect("impossible to fail to parse");
            let index_cursor = Cursor::new(index_vec);

            multipart.add_stream("index",  index_cursor,  None as Option<&str>, Some(index_mime));


            let metadata_str = match serde_json::to_string(&param_metadata) {
                Ok(str) => str,
                Err(e) => return Err(ApiError(format!("Unable to serialize metadata to string: {e}"))),
            };

            let metadata_vec = metadata_str.as_bytes().to_vec();
            let metadata_mime = mime::Mime::from_str("application/json").expect("impossible to fail to parse");
            let metadata_cursor = Cursor::new(metadata_vec);

            multipart.add_stream("metadata",  metadata_cursor,  None as Option<&str>, Some(metadata_mime));
        ...
        *request.body_mut() = body_from_string(body_string);
        ...
        match response.status().as_u16() {
            200 => {
                ...
                Ok(UpdateMergedImageResponse::SuccessfulOperation
                    (body)
                )
            }
            400 => {
                ...
                Ok(UpdateMergedImageResponse::BadRequest
                    (body)
                )
            }
            500 => {
                ...
                Ok(UpdateMergedImageResponse::InternalServerError
                    (body)
                )
            }
            code => {
        ...
        *request.body_mut() = body_from_string(body_string);
        ...
        match response.status().as_u16() {
            200 => {
                ...
                Ok(UploadImageResponse::SuccessfulOperation
                    (body)
                )
            }
            400 => {
                ...
                Ok(UploadImageResponse::BadRequest
                    (body)
                )
            }
            500 => {
                ...
                Ok(UploadImageResponse::InternalServerError
                    (body)
                )
            }
            code => {
    ...
    async fn upload_merged_image(
        ...
        param_signed_url: String,
        param_metadata: String,
        param_files: Option<&Vec<models::File>>,
        context: &C) -> Result<UploadMergedImageResponse, ApiError>
    {
        ...
        let (body_string, multipart_header) = {
            ...
            let files_str = match serde_json::to_string(&param_files) {
                Ok(str) => str,
                Err(e) => return Err(ApiError(format!("Unable to serialize files to string: {e}"))),
            };

            let files_vec = files_str.as_bytes().to_vec();
            let files_mime = mime::Mime::from_str("application/json").expect("impossible to fail to parse");
            let files_cursor = Cursor::new(files_vec);

            multipart.add_stream("files",  files_cursor,  None as Option<&str>, Some(files_mime));
        ...
        *request.body_mut() = body_from_string(body_string);
        ...
        match response.status().as_u16() {
            200 => {
                ...
                Ok(UploadMergedImageResponse::SuccessfulOperation
                    (body)
                )
            }
            400 => {
                ...
                Ok(UploadMergedImageResponse::BadRequest
                    (body)
                )
            }
            500 => {
                ...
                Ok(UploadMergedImageResponse::InternalServerError
                    (body)
                )
            }
            code => {
