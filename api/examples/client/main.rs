...
fn main() {
    ...
    match matches.get_one::<String>("operation").map(String::as_str) {
        Some("Ping") => {
            let result = rt.block_on(client.ping(
            ));
            info!("{:?} (X-Span-ID: {:?})", result, (client.context() as &dyn Has<XSpanIdString>).get().clone());
        },
        Some("UpdateMergedImage") => {
            let result = rt.block_on(client.update_merged_image(
                  "signed_url_example".to_string(),
                  56,
                  "metadata_example".to_string(),
                  swagger::ByteArray(Vec::from("BINARY_DATA_HERE"))
            ));
            info!("{:?} (X-Span-ID: {:?})", result, (client.context() as &dyn Has<XSpanIdString>).get().clone());
        },
        Some("UploadImage") => {
            let result = rt.block_on(client.upload_image(
                  "signed_url_example".to_string(),
                  "metadata_example".to_string(),
        ...
        Some("UploadMergedImage") => {
            let result = rt.block_on(client.upload_merged_image(
                  "signed_url_example".to_string(),
                  "metadata_example".to_string(),
