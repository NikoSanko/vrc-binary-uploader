...
impl<C> Api<C> for Server<C> where C: Has<XSpanIdString> + Send + Sync
{
    ...
    {
        info!("update_merged_image(\"{}\", {}, \"{}\", {:?}) - X-Span-ID: {:?}", signed_url, index, metadata, file, context.get().0.clone());
        Err(ApiError("Api-Error: Operation is NOT implemented".into()))
    }
    ...
    {
        info!("upload_image(\"{}\", \"{}\", {:?}) - X-Span-ID: {:?}", signed_url, metadata, file, context.get().0.clone());
        Err(ApiError("Api-Error: Operation is NOT implemented".into()))
    }
    ...
    {
        info!("upload_merged_image(\"{}\", \"{}\", {:?}) - X-Span-ID: {:?}", signed_url, metadata, files, context.get().0.clone());
        Err(ApiError("Api-Error: Operation is NOT implemented".into()))
    }
